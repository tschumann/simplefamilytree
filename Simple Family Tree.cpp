/*
C source code compiled with Microsoft's Visual C/C++ 2008
jdmcox.com/Simple Family Tree.rc
jdmcox.com/Simple Family Tree.h (rename it to resource.h)
jdmcox.com/ijl15.dll
jdmcox.com/ijl15.zip
...
\n\
11 Apr 2015       go to jdmcox.com for updates\n\
...
int d, bd, date[1024], smallest, dateptr, Year, Year2;
...
DWORD u, v, w, saveW, x, y, z, I, X, firstX, tempY, prevy, SpaceToLeft, LeftmostSibling, FirstParent, Child, TotalParents, fileSize, fileSize2, PhotoFileSize, JpegFileSize, dwBytesRead, dwBytesWritten, wParameter, MouseLoc;
			...
			else if ((Buf[x+1] == '1') && (Buf[x+2] == ' ') && (Buf[x+3] == 'F') && (Buf[x+4] == 'A') && (Buf[x+5] == 'M'))
			{
				...
				{//FAMC
					...
					else
					{
						...
						ch = Buf[y];
						Buf[y] = 0;
						MessageBox(hwnd, TEXT("In the Gedcom data file, the above is a child of more than two couples!\nChange it in Notepad"), &Buf[Indiv[i].Name], MB_OK);
						Buf[y] = ch;
					}
				}
...
//		if (Indiv[IndivNum].Flags & 0x20)
//			CheckDlgButton(hwndIndiv, IDC_CHECK1, BST_CHECKED);
...
x15:;
			...
			{//link to an existing individual
				...
				if (DialogBox(hInst, TEXT("LIST"), hwnd, ListProc)) // get father, mother, child, or spouse from list
				{ // i comes from DialogBox
					i = iFromList;
					if ((newfather) && (Indiv[i].Sex == 0xFFFF)) // sex of 1 is female
					{
						newfather = FALSE;
					...
					else if ((newmother) && (Indiv[i].Sex == 1)) // sex of 0xFFFF is male
					{
						newmother = FALSE;
						for (y = Indiv[i].Name, z = 0; (y < fileSize) && (Buf[y] != '\r') && (Buf[y] != '\n'); y++)
							if (Buf[y] != '/')
					...
					{//ask which couple to add the child to, and add the appropriate FAMC to the child's record
						...
						for (s = 0; (s < MAXSPOUSES) && (Indiv[IndivNum].Spouseof[s]); s++)
						{
							...
							for (I = 0; I < LastIndiv; I++)
							{
								if (I == IndivNum)
									continue;
								for (u = 0; (u < MAXSPOUSES) && (Indiv[I].Spouseof[u]); u++)
								{
									if ((Indiv[I].Spouseof[u] & 0x7FFFFFFF) == (Indiv[IndivNum].Spouseof[s] & 0x7FFFFFFF))
									{
										notherspouse = FALSE;
										foundspouse = TRUE;
										...
										if (Indiv[IndivNum].Sex != Indiv[I].Sex)
										{
											if (IDYES == MessageBox(hwnd, ParentsName, ChildsName, MB_YESNO))
											{
												...
												FamNum = Indiv[I].Spouseof[u] & 0x7FFFFFFF;
												PutFAM(CHILD, NewIndivOffset);
												PutFAMC(NewIndivOffset);
												goto x17;
											}
//											else break; // shouldn't be necessary
										}
									}
								}
							}
...
//				DeleteOffset++;//DeleteOffset is beginning of individual's record in Buf
			...
			//fall thru...
		case IDOK:
//			if (BST_CHECKED == IsDlgButtonChecked(hwndIndiv, IDC_CHECK1))
//				Indiv[IndivNum].Flags |= 0x20;
//			else
//				Indiv[IndivNum].Flags &= 0xFFFFFF0F;
			FillhdcMem();
			anyeditit = FALSE;
			preditit = FALSE;
			if (editit)
...
void SortSiblings(void)
{
	DWORD x, y, z, xBirth, yBirth, Parentof[2];
	...
	for (x = 0; x < i; x++)
	{
		if (Indiv[x].Childof)
		{
			...
			// check for single-parent child with half-siblings
			Parentof[0] = Parentof[1] = 0;
			for (y = 0; y < i; y++)
			{
				for (z = 0; z < MAXSPOUSES; z++)
				{
					if (Indiv[y].Spouseof[z] == Indiv[x].Childof)
					{
						if (!Parentof[0]) Parentof[0] = y;
						else Parentof[1] = y;
					}
						
				}
			}
			if (Parentof[1] == 0) // only one parent, so if there are siblings, sort this one to the left (as younger ones are)
			{
				y = Parentof[0]; // Indiv[y] is single parent
				if (Indiv[y].Spouseof[0] != Indiv[x].Childof) {
					for (z = 0; Indiv[y].Spouseof[z] != Indiv[x].Childof; z++)
						;
					for ( ; z > 0; z--)
						Indiv[y].Spouseof[z] = Indiv[y].Spouseof[z-1];
					Indiv[y].Spouseof[0] = Indiv[x].Childof;
				}
			}
		}
	}
	...
	for (x = 1; x < LastIndiv; x++)
	{
		for (y = 0; (y < MAXSPOUSES) && (Indiv[x].Spouseof[y]) && (f < 5000); y++)
		{
			...
			if (gotit == FALSE)
			{
				...
				MessageBox(hwnd, TEXT("doesn't refer to anything in the Family part of the the Gedcom data file.\nYou should delete that line in Notepad."), Error, MB_OK);
			}
		}
	}
/*
	...
	for (z = 0; z < FamsEnd; z++)
	{
		...
		{//at "0 @F"
			...
			if (gotit == FALSE)
			{
				_snwprintf(Error, 512, TEXT("the line 0 @F%i@ FAM"), fam);
